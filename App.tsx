...
const App: React.FC = () => {
  ...
  if (!IS_CONFIGURED && !showSplash) {
    return (
      <div className="flex items-center justify-center h-screen parchment-bg p-8 text-center font-inter">
        <div className="parchment-overlay" />
        <div className="max-w-md bg-white/60 backdrop-blur-xl p-12 rounded-[2.5rem] border border-amber-900/20 shadow-2xl z-20">
          ...
          <a href="https://aistudio.google.com/app/apikey" target="_blank" className="block w-full bg-amber-900 text-amber-50 py-4 rounded-2xl font-bold uppercase tracking-widest text-xs shadow-lg hover:bg-amber-800 transition-all">Get Gemini Key</a>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-screen parchment-bg overflow-hidden font-inter text-sm md:text-base">
      ...
      <main className="flex-1 flex flex-col relative">
        <header className="h-16 flex items-center justify-between px-6 z-20 border-b border-amber-900/5 backdrop-blur-sm bg-white/10">
          ...
          <button onClick={handleClearCurrentChat} className={`p-2 transition-colors ${messages.length > 1 ? 'text-amber-900/60 hover:text-red-600' : 'text-amber-900/10'}`} disabled={messages.length <= 1}><Trash2 className="w-5 h-5" /></button>
        </header>
          ...
          <div className="max-w-3xl mx-auto space-y-12 pb-40">
            {messages.length === 1 && (
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-12">
                 {SUGGESTED_TOPICS.map(t => (
                   <button key={t} onClick={() => handleSend(t)} className="p-6 text-left bg-white/40 hover:bg-amber-900/5 border border-amber-900/10 rounded-3xl transition-all shadow-sm hover:shadow-md active:scale-98">
            ...
            {messages.map((m, idx) => (
              <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
                <div className={`max-w-[90%] group ${m.role === 'user' ? 'bg-amber-900/10 p-5 rounded-3xl rounded-tr-none border border-amber-900/10' : 'w-full message-model p-8 shadow-sm'}`}>
                  {m.role === 'model' ? (
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[#fcf5e5] via-[#fcf5e5]/90 to-transparent z-20">
          <div className="max-w-3xl mx-auto">
             <div className="flex items-center gap-3 bg-white/60 backdrop-blur-md border border-amber-900/10 rounded-[2rem] p-3 shadow-2xl focus-within:border-amber-900/30 transition-all">
               ...
               <button onClick={toggleListening} className={`p-4 rounded-2xl transition-all active:scale-95 ${isListening ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-amber-900/10 text-amber-900 hover:bg-amber-900/20'}`}>
                 {isListening ? <MicOff className="w-5 h-5" /> : <Mic className="w-5 h-5" />}
               </button>
               <button onClick={() => handleSend()} disabled={!input.trim() || isGenerating} className="p-4 bg-amber-900 disabled:bg-slate-300 text-amber-50 rounded-2xl hover:bg-amber-800 shadow-xl transition-all active:scale-95">
